...
def show_summary(conn):
    ...
    total_bal = conn.execute("SELECT SUM(balance) AS sum FROM users WHERE active=1").fetchone()["sum"] or 0
    auth_ok   = conn.execute("SELECT COUNT(*) AS count FROM auth_log WHERE event_type LIKE '%OK%'").fetchone()["count"]
    auth_fail = conn.execute("SELECT COUNT(*) AS count FROM auth_log WHERE event_type LIKE '%FAIL%'").fetchone()["count"]
